...
- Promise 문제점 예시 코드
  ...
  function youVisitAppleStore(data) {
    return new Promise((resolve, reject) => {
  ...
  newMacBookOrder()
    ...
    .then((result2) => {
      console.log(result2);
      return youVisitAppleStore(result2);
    })
    .then((result3) => {
      console.log(result3);
    })
...
async function AppleStoreOrder() {
  try {
    ...
    await youVisitAppleStore(result2);
  } catch (error) {
    console.log(error);
  }
}
...
여기서는 value가 1,2,3인 경우 done: false지만 더 이상 반복이 없는 4번째 출력문에선는 value가 undefined done가 true로 바뀌어 있는 것을 알 수 있다.
